...
folders <- list.files("./transposed_files/", full.names = TRUE, recursive = TRUE)[str_detect(list.files("./transposed_files/"), "datafiles")]
## then get each file that we want in the folder
juv_file <- pre_file <- adult_file <- data.frame()
for (i in 1:length(folders)) {
  temp <- read_xlsx(folders[i])
  if (str_detect(folders[i], "Adult")) {
      adult_file <- bind_rows(temp, adult_file)
  }
  else if (str_detect(folders[i], "Juvenile")) {
      juv_file <- bind_rows(temp, juv_file)
  }
  else if (str_detect(folders[i], "Pretrial")) {
      pre_file <- bind_rows(temp, pre_file)
  }
}
